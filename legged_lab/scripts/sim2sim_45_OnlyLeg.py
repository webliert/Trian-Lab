...
class MujocoRunner:
    ...
    def init_variables(self) -> None:
        ...
        self.default_dof_pos = np.array(
            [0, -0.5, 0, 1.0, -0.5, 0, 0, -0.5, 0, 1.0, -0.5, 0]
        )
    ...
    def get_obs(self) -> np.ndarray:
        ...
        self.dof_pos = self.data.sensordata[0:12]
        self.dof_vel = self.data.sensordata[20:32]
        ...
        obs = np.concatenate(
            [
                ...
                self.command_vel,  # 3
                (self.dof_pos - self.default_dof_pos),  # 20
                self.dof_vel,  # 20
    ...
    def position_control(self) -> np.ndarray:
        ...
        # 创建20维的控制信号，前12维使用action，后8维设为0（手臂保持默认位置）
        full_control = np.zeros(20)
        full_control[:12] = actions_scaled + self.default_dof_pos[:12]
        full_control[12:] = self.default_arm_pos  # 手臂保持默认位置
