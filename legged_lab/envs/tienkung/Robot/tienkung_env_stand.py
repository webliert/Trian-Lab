...
from legged_lab.envs.tienkung.Experiment.stand_cfg import TienKungStandFlatEnvCfg
from legged_lab.utils.env_utils.scene import SceneCfg
from rsl_rl.env import VecEnv


class TienKungStandEnv(VecEnv):
    """
    天工机器人站立环境类 / TienKung Robot Standing Environment Class
    
    继承自VecEnv，实现天工机器人的站立仿真环境，不需要gait参数。
    Inherits from VecEnv, implements standing simulation environment for TienKung robot without gait parameters.
    """

    def __init__(
        self,
        cfg: TienKungStandFlatEnvCfg,
        headless,
    ):
        """
        ...
        Args:
            cfg: 环境配置对象 / Environment configuration object
            headless: 是否无头模式 / Whether to run in headless mode
        """
        self.cfg: TienKungStandFlatEnvCfg = cfg
    ...
    """
    天工机器人站立环境实现模块 / TienKung Robot Standing Environment Implementation Module
    
    该模块实现了天工机器人的站立强化学习环境，不需要gait参数。
    This module implements the standing reinforcement learning environment for TienKung robot without gait parameters.
    """
    ...
    def init_buffers(self):
        ...
        # Init gait parameter - 使用固定值用于站立（不需要实际步态）
        self.gait_phase = torch.zeros(self.num_envs, 2, dtype=torch.float, device=self.device, requires_grad=False)
        self.gait_cycle = torch.ones(self.num_envs, dtype=torch.float, device=self.device, requires_grad=False)
        # 站立时两脚相位相同（着地）
